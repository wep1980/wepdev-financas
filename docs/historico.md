...
verdade do `usuario.teste` (R$ 32.508,67, soma de 9 contas reais) e o
...
final: upload real da fatura de teste (`Nubank_2026-07-17.pdf`) via
...
usada nos testes deste projeto (titular "Waldir Escouto Pereira" +
uma segunda pessoa). Atualizado `firstName`/`lastName` do usuário pra
"Waldir"/"Escouto Pereira" (no realm import `realm-financas.json` e via
...
filtro automático pelo nome "Waldir Escouto Pereira") — os 2 removidos
são da segunda pessoa da fatura. Confirmação gerou exatamente 1
`Transacao` ("Fatura de cartão de crédito — Nubank_2026-07-17", DESPESA,
R$ 984,10) e o saldo da conta caiu de R$ 4.866,67 pra R$ 3.882,57 —
...
(`Nubank_2026-07-17.pdf`) com `cartaoId` obrigatório → 11 lançamentos
...
o upload da fatura Santander (`Fatura_082026_JUSSARA_...pdf`) de verdade
